...
window.SumiInkBlob = function SumiInkBlob({
  accentColor = '#c9533a',
  fgColor = '#3a2e26',
  opacity = 1,
  scale = 1,
  ...
  useEffect(() => {
    ...
    function draw(now) {
      ...
      const cx = w / 2, cy = h / 2;
      const pulse = 1 + 0.06 * Math.sin(time * 0.0008);
      const baseR = Math.min(w, h) * 0.32 * pulse * scale;

      const innerGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, baseR * 0.6);
      innerGrad.addColorStop(0, hexToRgba(accentColor, 0.10 * opacity));
      innerGrad.addColorStop(0.5, hexToRgba(accentColor, 0.04 * opacity));
      innerGrad.addColorStop(1, hexToRgba(accentColor, 0));
      ...
      for (let i = 0; i < 14; i++) {
        const angle = (i / 14) * Math.PI * 2 + time * 0.00006;
        const offsetR = baseR * 0.18 * (1 + 0.3 * Math.sin(time * 0.0003 + i * 0.7));
        const x = cx + Math.cos(angle) * offsetR;
        const y = cy + Math.sin(angle) * offsetR;
        const r = baseR * (0.55 + 0.2 * Math.sin(time * 0.0004 + i * 1.3));

        const grad = ctx.createRadialGradient(x, y, 0, x, y, r);
        grad.addColorStop(0, hexToRgba(fgColor, 0.05 * opacity));
        grad.addColorStop(0.6, hexToRgba(fgColor, 0.025 * opacity));
        grad.addColorStop(1, hexToRgba(fgColor, 0));
        ctx.fillStyle = grad;
        ctx.beginPath();
  ...
  }, [accentColor, fgColor, opacity, scale]);
